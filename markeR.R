...
markeR <- function(seurat_obj, output_dir, reduction_to_use, saveRData, dot_topN_roc, dot_topN_wilcox, feat_topN_wilcox, feat_topN_roc){
  ...
  mar_s <- FindAllMarkers(seurat_obj,
                          test.use = "roc",
                          only.pos = TRUE,
                          random.seed = 10086,
                          assay = "SCT",
  ...
  mar_wil <- FindAllMarkers(seurat_obj,
                            test.use = "wilcox",
                            only.pos = TRUE,
                            random.seed = 10086,
                            assay = "SCT",
  ...
  ## roc
   mar_s_top <- mar_s %>%
    group_by(cluster) %>%
    slice_max(order_by = avg_log2FC, n = dot_topN_roc, with_ties = TRUE) %>%
    ungroup() %>%
    arrange(cluster, desc(avg_log2FC))
  
  pd <- scCustomize::DotPlot_scCustom(seurat_obj,
  ...
  png(filename = file.path(output_dir, paste0("Dotplot_top", dot_topN_roc, "_markers_ROC.png")), 
      width = max(40 * num_clusters + 100, 1100), height = 25 * dot_topN_roc * num_clusters + 100, res = 110)
  print(pd)
  dev.off()
  ...
  mar_wil_top <- mar_wil %>%
    group_by(cluster) %>%
    slice_max(order_by = avg_log2FC, n = dot_topN_wilcox, with_ties = TRUE) %>%
    ungroup() %>%
    arrange(cluster, desc(avg_log2FC))
  
  pe <- scCustomize::DotPlot_scCustom(seurat_obj,
  ...
  png(filename = file.path(output_dir, paste0("Dotplot_top", dot_topN_wilcox, "_markers_Wilcox.png")), 
      width = max(40 * num_clusters + 100, 1100), height = 25 * dot_topN_roc * num_clusters + 100, res = 110)
  print(pe)
  dev.off()
  ...
  # top feature plot
  ## roc
  folder_name <- file.path(output_dir, paste0("FeaturePlot_top", feat_topN_roc, "_markers_ROC"))
  ...
  for (i in unique(marker_top$cluster)){
    ...
    for (k in seq(1, length(genes))){
      if (k %in% seq(1, length(genes), 20)){
        ...
        p <- scCustomize::FeaturePlot_scCustom(seurat_obj,
                                               raster = FALSE,
        ...
        png(filename = file.path(folder_name, paste0("cluster_", i, "_top", k, "-", (k+19), "_features.png")), 
            width = 3100, height = img_height)
        print(p)
        dev.off()
      }
    }
  ...
  ## wilcox
  folder_name <- file.path(output_dir, paste0("FeaturePlot_top", feat_topN_wilcox, "_markers_Wilcox"))
  if (!dir.exists(folder_name)) {
    dir.create(folder_name, recursive = TRUE, showWarnings = FALSE)
  }
...
option_list <- list(
  ...
  make_option(c("--reduction_to_use"), type = "character", default = "umap",
              help = "Reduction to use for feature plots (default: umap)"),
  make_option(c("--saveRData"), type = "logical", default = TRUE,
              help = "Whether to save the marker results as RData"),
  make_option(c("--dot_topN_roc"), type = "integer", default = 5,
              help = "Number of top markers to display in the ROC dot plot"),
  make_option(c("--dot_topN_wilcox"), type = "integer", default = 5,
              help = "Number of top markers to display in the Wilcox dot plot"),
  make_option(c("--feat_topN_wilcox"), type = "integer", default = 200,
              help = "Number of top markers to display in the Wilcox feature plot"),
  make_option(c("--feat_topN_roc"), type = "integer", default = 20,
...
markeR(seurat_obj = opt$seurat_obj,
